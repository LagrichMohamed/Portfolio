            ...
            <div key={index} className='mb-8 flex flex-wrap lg:justify-center'>
               ...
               className='w-full max-w-xl lg:w-3/4'>
                  <h6 className='mb-2 text-xl font-semibold'>
                     {experience.role} - <span className='text-xl text-purple-100'>{experience.company}</span>
                  </h6>
                  <p className='mb-4 text-lg text-neutral-400'>
                     {experience.description}
                  </p>
                  {experience.technologies.map((tech, index) => (
                     <span key={index} className='mr-2  rounded bg-neutral-900 px-2 py-1 text-md  font-medium text-purple-800 '>{tech} </span>
                  ))}
               </motion.div>
            </div>
         ))}
